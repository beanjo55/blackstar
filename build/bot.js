...
class Bot {
    ...
    async firstInit() {
        ...
        mongoose_1.default.connection.on("open", () => signale_1.default.success("Connected to mongoDB!"));
        mongoose_1.default.connection.on("error", () => signale_1.default.error("Failed to connect to mongoDB!"));
    ...
    async init() {
        ...
        if (!temp) {
            signale_1.default.fatal("Could not load global config");
            process.exit(0);
        }
        else {
            this.global = temp;
    ...
    async messageDelete(msg) {
        ...
        const guild = this.client.guilds.get(msg.channel.guild.id);
        if (data) {
            if (data.post) {
    ...
    async messageReactionAdd(omsg, emote, user) {
        ...
        if (!(user instanceof eris_1.Member)) {
        ...
        if (!data.post) {
            ...
            if (data.count >= threshold) {
                ...
                if (!destChannelObj || (!(destChannelObj.type === 0 || destChannelObj.type === 5))) {
                    return;
                }
            ...
            if (data.count >= threshold) {
                ...
                if (!destChannel || (!(destChannel.type === 0 || destChannel.type === 5))) {
                    return;
                }
    ...
    async messageCreate(msg) {
        ...
        switch (name.toLowerCase()) {
            ...
            case "e":
            case "eval": {
                if (msg.author.id !== "253233185800847361") {
                    break;
                }
